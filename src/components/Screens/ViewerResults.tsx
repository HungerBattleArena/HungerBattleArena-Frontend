...
export default function ViewerResults({ victory }: ViewerResultsProps) {
  ...
  const totalPool = selectedRoom?.totalBet || 0;
  ...
  const totalBetSideWin =
    winningSide === "WIN"
      ? selectedRoom?.winBet || 0
      : selectedRoom?.loseBet || 0;
  ...
  return (
    <div className="absolute inset-0 bg-black/95 pointer-events-auto z-60 flex flex-col items-center justify-center fade-in">
      ...
      <p
        className={`text-xl mb-12 tracking-[0.5em] uppercase ${userWon ? "text-green-400" : "text-red-500"
          }`}
      >
      ...
      <div className="flex flex-col md:flex-row gap-12 w-full max-w-5xl">
        <div
          className={`w-full md:w-1/2 glass-panel p-8 border-l-4 ${winningSide === "WIN"
              ? "border-cyan-500 win-glow-savior"
              : "border-pink-500 win-glow-doomer"
            } transition duration-1000`}
        >
          <h3 className="text-2xl text-cyan-400 mb-6 border-b border-gray-700 pb-2">
            POOL SUMMARY
          </h3>
          <div className="space-y-4 font-mono text-sm">
            ...
            <div className="flex justify-between">
              <span className="text-gray-400">FIGHTER REWARD (15%)</span>
              <span className="text-yellow-400">
                {animatedValues.fighterReward.toLocaleString()}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-400">WINNING SIDE POOL</span>
              <span className="text-green-400 text-xl font-bold">
                {animatedValues.viewerPool.toLocaleString()}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="w-full md:w-1/2 glass-panel p-8 border-l-4 border-gold-400 relative overflow-hidden">
          <h3 className="text-2xl text-gold-400 mb-6 border-b border-gray-700 pb-2">
            YOUR PAYOUT
          </h3>
          <div className="space-y-5">
            ...
            <div className="bg-black/40 p-4 border border-gray-700 rounded text-center">
              ...
              <div className="text-5xl font-black text-white">
                {animatedValues.userPayout.toLocaleString()}
              </div>
            </div>
            <div className="flex justify-between items-end">
              <span className="text-gray-400 text-sm">PNL</span>
              <span
                className={`font-mono text-xl ${pnl >= 0 ? "text-green-400" : "text-red-500"}`}
              >
                {pnl.toLocaleString()}
              </span>
            </div>
            {pnl > 0 && (
              <button className="btn-cyber px-10 py-3 text-lg font-bold">
                Claim
              </button>
            )}
          </div>
      ...
      <button
        className="btn-cyber px-12 py-4 mt-16 text-xl font-bold"
        onClick={() => navigate("/")}
      >
        Return to Lobby
      </button>
